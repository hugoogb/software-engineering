...
{
  int CodiBreaker[LongCodi], IntCodiBreaker, FrequenciaCodiMaster[Digits], FrequenciaCodiBreaker[Digits], Encerts = 0, Aproximacions, Intents = 0;
...
void ImprimirResultatPartida(char NomJugador1[], char NomJugador2[], RegistrePartida Resultat, int NumPartida)
{
  ...
  {
    cout << "Ronda " << NumPartida << ": Empat, cap dels jugadors ha encertat el codi secret de l'altre." << endl;
  }
  else if (Resultat.IntentsJugador1 == Resultat.IntentsJugador2)
  {
    cout << "Ronda " << NumPartida << ": Empat, ambdos jugadors han encertat l'altre codi amb " << Resultat.IntentsJugador1 << " intents." << endl;
  }
  else if ((Resultat.IntentsJugador2 < Resultat.IntentsJugador1 && Resultat.IntentsJugador2 != 0) || (Resultat.IntentsJugador2 != 0 && Resultat.IntentsJugador1 == 0))
  {
    cout << "Ronda " << NumPartida << ": Guanya " << NomJugador2 << " encertant el codi ";
    EscriureVector(Resultat.CodiJugador1, LongCodi);
    cout << " amb " << Resultat.IntentsJugador2 << " intents." << endl;
  }
  else if ((Resultat.IntentsJugador1 < Resultat.IntentsJugador2 && Resultat.IntentsJugador1 != 0) || (Resultat.IntentsJugador1 != 0 && Resultat.IntentsJugador2 == 0))
  {
    cout << "Ronda " << NumPartida << ": Guanya " << NomJugador1 << " encertant el codi ";
    EscriureVector(Resultat.CodiJugador2, LongCodi);
    cout << " amb " << Resultat.IntentsJugador1 << " intents." << endl;
  }
}

void ImprimirHistoric(char NomJugador1[], char NomJugador2[], RegistrePartida RegistreHistoric[], int PartidesJugades)
{
  if (PartidesJugades == 0)
  {
    cout << "Encara no s'ha jugat cap partida i no hi ha cap historic a mostrar." << endl;
  }
  else
  {
...
int main()
{
  ...
  int opcio;
  int numPartides = 0;
  cout << "Benvinguts al joc MasterMind per a 2 jugadors.Si us plau, introduiu els vostres noms: ";
